...
export function EstateQueryPanel({ estateId, onNodeFocus }: Props) {
  ...
  return (
    <>
      {/* Trigger button — sits in the estate HUD area */}
      <button
        className="eq-trigger"
        onClick={() => setOpen((o) => !o)}
        aria-label="Query estate"
        title="Ask a question about this estate"
      >
      ...
      {open && (
        <div className="eq-backdrop" onClick={() => setOpen(false)} />
      )}
      ...
      <aside className={`eq-panel${open ? " eq-panel--open" : ""}`} role="dialog">
        <header className="eq-panel__head">
          ...
          <button className="panel__close" onClick={() => setOpen(false)}>✕</button>
        </header>
        ...
        <div className="eq-panel__body">
          ...
          {error && (
            <div className="panel__error" style={{ marginTop: 12 }}>
              {error}
            </div>
          )}
