...
// =============================================================================
// AnalysisPanel — estate-side analysis surface.
// All structured rendering (the four JSON-mode cockpits + the modernize
// markdown renderer) lives in the shared module at
// components/analysis/cockpits.tsx, imported by both this panel and the
...
// Scorecards (summary) are surface-owned styling; the structured body below them
// is the shared Cockpit. Raw JSON stays one collapsed toggle away.
function Result({ mode, result }: { mode: AnalysisMode; result: unknown }) {
  ...
  // ── Modernize: streamed markdown → prose + labeled code panes ──
  if (mode === "modernize") {
  ...
  // ── JSON modes: scorecards + structured cockpit + raw toggle ──
  ...
  return (
    <div className="result">
      ...
      {details && <Cockpit mode={mode} details={details} />}
